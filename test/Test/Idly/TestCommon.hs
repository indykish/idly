...
{-| Common helper functions and instances for all Idly tests.

-}


module Test.Idly.TestCommon
  ( maxMem
  , maxDsk
  , maxCpu
  , maxSpindles
...
import           Idly.JSON               (ArrayObject (..))
import           Idly.Types
import           Idly.Utils.Monad        (unfoldrM)
...
-- the newUUID function in Idly.Utils
genUUID :: Gen String
genUUID = do
  c1 <- vector 6
  c2 <- vector 4
...
-- Idly's own tag regex, just enough for it to be close.
tagChar :: String
tagChar = ['a'..'z'] ++ ['A'..'Z'] ++ ['0'..'9'] ++ ".+*/:@-"
...
-- | Return the source directory of Idly.
